...
var OAuthAdapter = function(pConsumerSecret, pConsumerKey, pSignatureMethod) {
	...
	this.getPin = function() {
		...
		var done = !1, client = Ti.Network.createHTTPClient({
			onload : function() {
				Ti.API.info("0"+this.responseText);
	...
	var destroyAuthorizeUI = function() {
		if (window == null)
			return;
		try {
			Ti.App.fireEvent('login_closed',{what:'linkedin'});
	...
	this.showLoadingUI = function() {
		...
		var pUrl = "http://api.linkedin.com/v1/people/~:(first-name,last-name,headline,picture-urls::(original))?format=json";
		...
		client = Ti.Network.createHTTPClient({
			onload : function() {
				if (client.status == 200) {
					//Ti.API.info("HEREEEEEEEE "+this.responseText);
					var parsed = JSON.parse(this.responseText);
					Titanium.Network.removeAllHTTPCookies();
					Titanium.Network.removeAllSystemCookies();
					Ti.App.Properties.setString('first_name',parsed.firstName);
					Ti.App.Properties.setString('last_name',parsed.lastName);
					Ti.App.Properties.setString('mail','');
					Ti.App.Properties.setString('id','');
					Ti.App.Properties.setString('login_type','linkedin');
					Ti.App.Properties.setString('picture', parsed.pictureUrls.values[0]);
					Ti.API.info(parsed);
					Ti.App.fireEvent('goto_page',{page:2});
					saveHer.stopExecution = false;
					//Ti.App.fireEvent('login_ok',{info:this.responseText, what:'linkedin'});
					//pSuccessMessage && pSuccessMessage(this.responseText);
				} else {
					Ti.API.info("Error Message");
					var dialog = Ti.UI.createAlertDialog({
   						message: "C\'è stato un errore nell\'elaborazione della richiesta. Riprova per favore.",
    					ok: 'Ok',
    					title: 'Linkedin'
  					}).show();
		...
		client = Ti.Network.createHTTPClient({
			onload : function() {
				if (client.status == 200 || client.status == 201) {
					Ti.API.info("3"+this.responseText);
					pSuccessMessage && pSuccessMessage(this.responseText);
				} else {
					pErrorMessage && pErrorMessage(this.responseText);
				}
		...
		var parameterMap = OAuth.getParameterMap(message.parameters), client = Ti.Network.createHTTPClient({
			onload : function() {
				Ti.API.info("4"+this.responseText);
